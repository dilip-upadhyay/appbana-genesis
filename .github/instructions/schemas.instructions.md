...
- Contains: use cases, personas, rules in plain language, workflow as prose or timeline, user journeys as prose, non-functional intents ("should be fast", "must be SOX-compliant").
...
- Contains: typed entities with fields and classifications, explicit role definitions, formal rule expressions, state machine with states/guards/transitions/effects, operation references, and — since [ADR-018](../../docs/adr/ADR-018-presentation-intent-ownership.md) — `interactionFlows[]`, the canonical form of presentation intent.
- `interactionFlows` is **medium-neutral**: `flow` / `step` / `group` / `placement`, deliberately not `screen` / `section` / `field-binding`. It says what a person is being asked to do and in what grouping, never how many pixels wide the box is. The rename to screen vocabulary happens in the CAM Generator, which is where a rendering medium is finally assumed.
...
|---|---|---|
| `InteractionModel` | UI Runtime | Screens, fields, field types, layout, conditional visibility, plus `origin` recording who authored the layout |
...
- Schema file naming: `{model-name}.v{major}.{minor}.schema.json` (e.g., `cam.v0.2.schema.json`). There is no unversioned alias — every consumer names the exact version it targets.
...
```
docs/schemas/
  bim.v0.1.schema.json     ← frozen, superseded
  bim.v0.2.schema.json     ← current
  aim.v0.1.schema.json     ← frozen, superseded
  aim.v0.2.schema.json     ← current
  cam.v0.1.schema.json     ← frozen, superseded
  cam.v0.2.schema.json     ← current
  operation-contract.v0.1.schema.json
  trace-event.v0.1.schema.json
  ai-adapter-manifest.v0.1.schema.json
```

Superseded versions are never edited or deleted. A published `$id` is a promise: an artifact generated against v0.1 must still be checkable years later, which is impossible if the file it validated against was rewritten. Only the current version is registered in `tools/validate-schemas/schemas.manifest.json` and exercised by CI.

TypeScript types **should** be generated from these schemas into `packages/*/src/generated/`. They are not yet — every `types.ts` in the repo is hand-written. See [DEV-005](../../docs/deviations.md). Do not describe the codegen as if it exists.
