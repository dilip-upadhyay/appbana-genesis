...
| AI Provenance Store | 4 d | Append-only Postgres table (`ai_provenance`) + query API; kernel refuses to consume any AI output whose provenance is missing or invalid. | ✅ Done (2026-07-24) — [`@appbana/ai-provenance-store`](../../packages/ai-provenance-store/) v0.1.0; 26/26 tests green (hash + assert + in-memory + JSONL); ships `AIProvenanceStore` interface + `InMemoryAIProvenanceStore` (tests) + `JsonlAIProvenanceStore` (durable, air-gapped) + Postgres DDL at [`sql/ai_provenance.sql`](../../packages/ai-provenance-store/sql/ai_provenance.sql) with RLS INSERT+SELECT-only; content-addressed ids dedupe replays; kernel guard `assertProvenance(result, stored)` throws `MissingProvenanceError` on missing/broken records or store-id mismatch; `listReferencedPromptVersions()` feeds `prompt-registry-check --provenance-refs` end-to-end (Postgres driver deferred to v0.2) | Dilip |
